...
try
    ...
    for i=1:1
        ...
        % fname= '/mnt/squid/RO-C-RPCLAP-5-1609-DERIV-V0.5/2016/SEP/D30/RPCLAP_20160930_000052_710_V1L.TAB';
         %fname= '/Users/frejon/Rosetta/EOM/RPCLAP_20160930_000052_710_V2L.TAB';

         fname= '/mnt/squid/RO-C-RPCLAP-5-1609-DERIV-V0.5/2016/SEP/D20/RPCLAP_20160920_172156_710_V2L.TAB';
        ...
        notlf_ind=dt_array<1; % check if it's  less than 1 sec.
        if ~isempty(notlf_ind)  % delete non medium frequency measurements
        
            scantemp{1,1}(notlf_ind)    = [];
            scantemp{1,2}(notlf_ind)    = [];
            scantemp{1,3}(notlf_ind)    = [];
            scantemp{1,4}(notlf_ind)    = [];
            scantemp{1,5}(notlf_ind)    = [];

            if fileflag(2) =='3'
                scantemp{1,6}(notlf_ind)    = [];
            end
        end
        
        
        %scantemp{1,end-1}= scantemp{1,end-1} - scantemp2{1,end-1}; %line
...
%             end
            ...
            if reltime(n+1)-reltime(n)>dt*10 %large jump, new timer
