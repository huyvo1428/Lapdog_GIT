...
%
% NOTE: EOG = "Elias Odelstad geometry" (file)
% NOTE: The function is chosen such that it could be relatively easily be replaced by a function that
%       produces a geometry file from scratch (from SPICE; without EOG files).
% NOTE: Requires SPICE kernels to have already been loaded.
% NOTE: Code contains the naming convention for EOG files.
...
% EOG_files_dir  : Directory where EOG files can be located.
%
%
% ~FIXED BUG: Old code assumed that certain columns in EOG files have the correct meaning. They have been observed not to.
% Old code only worked for EOG files with header "UTC, X_CSO, Y_CSO, Z_CSO, Lat, Lon, SZA, SAA, CAA, SEA, CEA"
% (and similarily named columns for non-comet targets).
%
%=========================================================================
function geometry_createFileFromEOGFile(dest_dir, PDS_data, t, EOG_files_dir)
    %===============================================================================================
    ...
    EOG_filename = [upper(datestr(t, 'yyyy-mmm-dd')), 'orb.txt'];
    EOG_file_path = fullfile(EOG_files_dir, EOG_filename);
    
    % ASSERTION
    if ~exist(EOG_file_path, 'file')
        error('Can not find EOG geometry file "%s".\n', EOG_file_path)
    end
    
    TAB_file_info = create_TAB_file(dest_dir, PDS_data, EOG_file_path, t);
    create_LBL_file(PDS_data, TAB_file_info);
end



function TAB_file_info = create_TAB_file(dest_dir, PDS_data, EOG_file_path, t)
    fc = importdata(EOG_file_path);         % fc = file contents.
    
    %============================================
    % ASSERTION - Basic check on EOG file format
    %============================================
    if (size(fc.textdata, 2) ~= 11) | (size(fc.data, 2) ~= 10) | (size(fc.data, 1)+1 ~= size(fc.textdata, 1))
        error('Unexpected data in "Elias Odelstad geometry" (EOG) file. Different file format from what is expected.')
    end
    
    
    fileNumericColumnNameList = strtrim(fc.textdata(1,:));   % importdata assumes single-character delimiter ','. Therefore column names (might) contain whitespace.
    fileNumericColumnNameList(1) = [];  % Remove first column name ("UTC").
    
    % Identify columns by name and put them in named variables.
    % NOTE: EOG files seem to always use suffix _CSO, including for non-comet targets.
    % Example EOG file header (first row): "UTC, X_CSO, Y_CSO, Z_CSO, Lat, Lon, SZA, SAA, CAA, SEA, CEA"
    fileData = [];
    fileData.X_TSO = fc.data(:, get_str_index({'X_CSO'}, fileNumericColumnNameList));
    fileData.Y_TSO = fc.data(:, get_str_index({'Y_CSO'}, fileNumericColumnNameList));
    fileData.Z_TSO = fc.data(:, get_str_index({'Z_CSO'}, fileNumericColumnNameList));

    fileData.Lat   = fc.data(:, get_str_index({'Lat'},   fileNumericColumnNameList));
    fileData.Lon   = fc.data(:, get_str_index({'Lon'},   fileNumericColumnNameList));

    fileData.SZA   = fc.data(:, get_str_index({'SZA'},   fileNumericColumnNameList));
    fileData.SAA   = fc.data(:, get_str_index({'SAA'},   fileNumericColumnNameList));

    fileData.TAA   = fc.data(:, get_str_index({'CAA', 'AAA'}, fileNumericColumnNameList));
    fileData.SEA   = fc.data(:, get_str_index({'SEA'},        fileNumericColumnNameList));
    fileData.TEA   = fc.data(:, get_str_index({'CEA', 'AEA'}, fileNumericColumnNameList));
    ...
    TAB_file_info.N_rows = length(UTCstr_list);
    ...
    % Construct TAB filename and path.
    ...
    for i = 1:TAB_file_info.N_rows
        % IMPLEMENTATION NOTE: fprintf can print multiple rows for numeric variables, but not for strings.
        % Must therefore execute fprintf once for every row.
        % IMPORTANT NOTE: The choice of columns must match the LBL file!!!!
...
%             UTCstr_list{i, 1}, OBT_list(i), num_table(i, 1:10));
        line_length = fprintf(fid, ...
            '%23s, %16.6f, %14.7e, %14.7e, %14.7e, %14.7e, %14.7e, %14.7e, %14.7e, %14.7e, %14.7e, %14.7e\r\n', ...
            UTCstr_list{i, 1}, OBT_list(i), ...
            fileData.X_TSO(i), fileData.Y_TSO(i), fileData.Z_TSO(i), ...
            fileData.Lat(i),   fileData.Lon(i), ...
            fileData.SZA(i), ...
            fileData.SAA(i), ...
            fileData.TAA(i), ...
            fileData.SEA(i), ...
...
% Search a list of strings for one string. Match one out of a list of alternative strings.
%
% ASSUMES: Exactly one of the strings in matchingStrList matches exactly one of the strings in strList.
function iStr = get_str_index(matchingStrList, strList)
    iStr = [];
    for i = 1:length(matchingStrList)
        matchingStr = matchingStrList{i};
        iStr = [iStr, find(strcmp(matchingStr, strList))];   % NOTE: "find" return row vector.
    end
    
    if numel(iStr) == 1
        return
    elseif numel(iStr) > 1
        error('Found multiple matching strings (file column names).')
    end
end
...
% writes the LBL file.
% IMPORTANT NOTE: The choice of columns must match the TAB file!!!!
%===============================================================================================
