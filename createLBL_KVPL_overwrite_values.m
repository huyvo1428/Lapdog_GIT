...
%   overwrite_values
    ...
    for i_kv_src = 1:length(kvl_src.keys)
        ...
        key   = kvl_src.keys{i_kv_src};
        value = kvl_src.values{i_kv_src};
        i_kvl_dest = find(strcmp(key, kvl_dest.keys));        

        if isempty(i_kvl_dest)            
            error(sprintf('ERROR: Tries to set key that does not yet exist in kvl_dest: (key, value) = (%s, %s)', key, value));
        elseif length(i_kvl_dest) > 1            
            error(sprintf('ERROR: Found multiple keys with the same value in kvl_dest: (key, value) = (%s, %s)', key, value));            
        end
            
        kvl_dest.values{i_kvl_dest, 1} = value;      % No error ==> Set value.
