...
if(~index(tabind(1)).sweep); %% if not a sweep, do:
    ...
    for(i=1:len);
        ...
        step1 = find (diff(scantemp{1,4}(1:end)),1,'first') +1;       
        scantemp{1,1}(1:step1-1)    = [];
        scantemp{1,2}(1:step1-1)    = [];
        scantemp{1,3}(1:step1-1)    = [];
        scantemp{1,4}(1:step1-1)    = [];
        ...
        dlmwrite(filename2,curtemp,'-append','precision', '%14.7e');
        
        if (i==len)
            
            tabindex(end,4:6)= {scantemp{1,1}{end,1}(1:23),scantemp{1,2}(end),scanlength}; %one index for bias voltages
            tabindex(end+1,1:6)={filename2,strrep(filename2,tabfolder,''),tabind(1), scantemp{1,1}{end,1}(1:23),scantemp{1,2}(end),counttemp2};
