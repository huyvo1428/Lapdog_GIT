...
for b = 1:nob   % Loop through all ops blocks
    ...
    p1s = find([index(ob).sweep] & ([index(ob).probe] == 1)); %%returns indices of all sweeps for probe 1 for macro operation block 
        if(~isempty(p1s)) %% Create filename according to convention, start TAB//LBL file genesis
            

   
           filename = sprintf('%s/RPCLAP_%s_%s_%d_%s.TAB',derivedpath,datestr(index(p1s(1)).t0,'yyyymmdd'),datestr(index(p1s(1)).t0,'HHMMSS'),index(p1s(1)).macro,'B1S'); %%
           createTABLBL(filename,p1s,index);
        end
    
    p2s = find([index(ob).sweep] & ([index(ob).probe] == 2));

        if(~isempty(p2s))
            filename = sprintf('%s/RPCLAP_%s_%s_%d_%s.TAB',derivedpath,datestr(index(p2s(1)).t0,'yyyymmdd'),datestr(index(p2s(1)).t0,'HHMMSS'),index(p2s(1)).macro,'B2S'); %%
            createTABLBL(filename,p2s,index);
        end
    ...
    p1el = find([index(ob).lf] & [index(ob).efield] & ([index(ob).probe] == 1)); 
        if(~isempty(p1el)) %% Create filename, start TAB//LBL file genesis.
            filename = sprintf('%s/RPCLAP_%s_%s_%d_%s.TAB',derivedpath,datestr(index(p1el(1)).t0,'yyyymmdd'),datestr(index(p1el(1)).t0,'HHMMSS'),index(p1el(1)).macro,'V1L'); %%
            createTABLBL(filename,p1el,index);
        end
    
    p2el = find([index(ob).lf] & [index(ob).efield] & ([index(ob).probe] == 2));
    
        if(~isempty(p2el)) %% Create filename, start TAB//LBL file genesis.
            filename = sprintf('%s/RPCLAP_%s_%s_%d_%s.TAB',derivedpath,datestr(index(p2el(1)).t0,'yyyymmdd'),datestr(index(p2el(1)).t0,'HHMMSS'),index(p2el(1)).macro,'V2L'); %%
            createTABLBL(filename,p2el,index);
        end
    
    
    p1eh = find([index(ob).hf] & [index(ob).efield] & ([index(ob).probe] == 1));
        
        if(~isempty(p1eh)) %% Create filename, start TAB//LBL file genesis.
            filename = sprintf('%s/RPCLAP_%s_%s_%d_%s.TAB',derivedpath,datestr(index(p1eh(1)).t0,'yyyymmdd'),datestr(index(p1eh(1)).t0,'HHMMSS'),index(p1eh(1)).macro,'V1H'); %%
            createTABLBL(filename,p1eh,index);
        end
    
    p2eh = find([index(ob).hf] & [index(ob).efield] & ([index(ob).probe] == 2));
        if(~isempty(p2eh))
            filename = sprintf('%s/RPCLAP_%s_%s_%d_%s.TAB',derivedpath,datestr(index(p2eh(1)).t0,'yyyymmdd'),datestr(index(p2eh(1)).t0,'HHMMSS'),index(p2eh(1)).macro,'V2H'); %%
            createTABLBL(filename,p2eh,index);
        end
    ...
    p1nl = find([index(ob).lf] & ~[index(ob).efield] & ([index(ob).probe] == 1));
        if(~isempty(p1nl)) 
            filename = sprintf('%s/RPCLAP_%s_%s_%d_%s.TAB',derivedpath,datestr(index(p1nl(1)).t0,'yyyymmdd'),datestr(index(p1nl(1)).t0,'HHMMSS'),index(p1nl(1)).macro,'I1L'); %%
            createTABLBL(filename,p1nl,index);
        end
    p2nl = find([index(ob).lf] & ~[index(ob).efield] & ([index(ob).probe] == 2));
        if(~isempty(p2nl))
            filename = sprintf('%s/RPCLAP_%s_%s_%d_%s.TAB',derivedpath,datestr(index(p2nl(1)).t0,'yyyymmdd'),datestr(index(p2nl(1)).t0,'HHMMSS'),index(p2nl(1)).macro,'I2L'); %%
            createTABLBL(filename,p2nl,index);
        end
    p1nh = find([index(ob).hf] & ~[index(ob).efield] & ([index(ob).probe] == 1));
        if(~isempty(p1nh))
            filename = sprintf('%s/RPCLAP_%s_%s_%d_%s.TAB',derivedpath,datestr(index(p1nh(1)).t0,'yyyymmdd'),datestr(index(p1nh(1)).t0,'HHMMSS'),index(p1nh(1)).macro,'I1H'); %%
            createTABLBL(filename,p1nh,index);
        end
    p2nh = find([index(ob).hf] & ~[index(ob).efield] & ([index(ob).probe] == 2));
        if(~isempty(p2nh))
            filename = sprintf('%s/RPCLAP_%s_%s_%d_%s.TAB',derivedpath,datestr(index(p2nh(1)).t0,'yyyymmdd'),datestr(index(p2nh(1)).t0,'HHMMSS'),index(p2nh(1)).macro,'I2H'); %%
            createTABLBL(filename,p2nh,index);
        end
