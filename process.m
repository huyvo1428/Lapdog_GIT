...
%    
  ...
  sct0temp= cellfun(@(s) s(4:end-1), {index(ob(p2s)).sct0str}, 'uni',false);
  sct1temp= cellfun(@(s) s(4:end-1), {index(ob(p2s)).sct1str}, 'uni',false);
  sweept2 = [str2double(sct0temp); str2double(sct1temp)];
  clear sct0temp sct1temp
...
%            psd_p1eh = [psd_p1eh; mean(ts) (128/lens)^2 * psd'];
% 
%       end
%       
%     end
