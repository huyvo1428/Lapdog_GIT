...
for i=1:len1
    ...
    a1 = strfind(tempfiles(i).name,'I1S.TAB');
    a2 = strfind(tempfiles(i).name,'I2S.TAB');
...
%        rfolder = strrep(tabindex{an_ind(i),1},tabindex{an_ind(i),2},'');
        ...
        arID = fopen(rfile,'r');                   % Open sweep file.
        ...
        if arID < 0
            fprintf(1,'Error, cannot open CORRESPONDING B file %s\n', rfile);
            break
        end % if I/O error
...
%         an_tabindex{end,9} = row_bytes;
    ...
    fprintf(1,'Error at loop step %i, file %s',i,rfile);
    if ~isempty(k)
